...
describe('useCalculator', () => {
    ...
    it("returns validation error when submitting empty values", async () => {
      ...
      expect(result.current.error).toBe("First number is required.");
    ...
    it("submits and stores successful calculation snapshot", async () => {
      ...
      const { result } = renderHook(() =>
        useCalculator({ onSuccessfulCalculation }),
      );
    ...
    it("allows square root with only first number", async () => {
      calculateMock.mockResolvedValue({ result: 3 });
      const { result } = renderHook(() => useCalculator());

      act(() => {
        result.current.setForm({ a: "9", b: "", operation: "sqrt" });
      });
      ...
      expect(result.current.error).toBe("");
      expect(result.current.lastCalculation?.result).toBe(3);
    });
})
